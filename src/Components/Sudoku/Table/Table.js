...
export const Table = ({arrTable, idTable, dangerId, index, numbers}) => {
    ...
    return (<div id={idTable} className='main__table' style={styleTable}>
        {arrTable.map(({number, id} = arrTable) => <Number dangerId={dangerId} number={number} id={id} idTable={index} numbers={numbers}/>)}
    </div>)
}